...
class ABC:
    def __init__(self, df, nameDS, rs=1, prePros=False, model=None, solver=None):
        '''
        Returns the results of using the AdaBoost technique
        ...
        model: object, default=None
            ...
            in the variable `df`.
        solver: str, 'pca', 'ipca', 'lda', default=None
        ...
        print(f'Running AdaBoost for {nameDS}...')
        self.reg = {'ABC': AdaBoostClassifier()}
        self.params = {'ABC': {'n_estimators': [50, 100, 150, 200, 250, 300, 350,
        ...
        if prePros:
            # For this part to work it is completely necessary to have executed the code without preprocessing and to
            # use the following hyperparameters: prePros=True, model=abc_xX.best_model, solver='pca' or 'lda'.
            ...
            for name, reg in self.reg.items():
                grid_reg = GridSearchCV(reg,               # Regulators.
            ...
            print(f'Metrics for {nameDS} with AdaBoost...')

            self.AB_nP = self.best_model.fit(self.X_train, self.y_train)

            self.y_pred = self.AB_nP.predict(self.X_test)
